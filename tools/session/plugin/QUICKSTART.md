...
claude plugin marketplace add RileyLin/explainify@explainify-session-v0.4.1 \
...
marketplace at the immutable tag so the new plugin version (`0.4.1`) replaces
any cached older bytes, then restart:
...
claude plugin details explainify-session@explainify-local   # version 0.4.1
```
